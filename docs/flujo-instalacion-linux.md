...
flowchart TD
    ...
    %% ===== 7. BACKUP =====
    mission --> backup_check{¿Config opencode existe?}:::check
    ...
    engram_skip --> vscode_check
    engram_install --> vscode_check

    vscode_check:::check --> vscode_q{¿Instalar VSCode?}
    vscode_q -- Sí --> vscode_install_check{¿Ya instalado?}:::check
    vscode_install_check -- Sí --> vscode_skip[Skip]:::skip
    vscode_install_check -- No --> vscode_do[Instalar VSCode]:::install
    vscode_q -- No --> gga_check
    vscode_skip --> gga_check
    vscode_do --> gga_check

    gga_check:::check --> gga_q{¿Instalar
    Guardian Angel?}
    gga_q -- Sí --> gga_install_check{¿Ya instalado?}:::check
    gga_install_check -- Sí --> gga_skip[Skip]:::skip
    gga_install_check -- No --> gga_do[Instalar GGA]:::install
    gga_q -- No --> templates
    gga_skip --> templates
    gga_do --> templates
    ...
    en ~/.config/lara-diaries/]:::personal
